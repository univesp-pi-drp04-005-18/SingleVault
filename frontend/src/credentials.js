...
//       const response = await fetch(`${process.env.REACT_APP_API_URL}/credentials/${selectedCredential.name}`, {
//         method: 'PUT',
//         headers: {
//           'Authorization': `Bearer ${token}`,
//           'Content-Type': 'application/json',
...
const PasswordGenerator = ({ 
  ...
  return (
    <div style={{ 
      ...
      <h4>Gerador de Senha</h4>
      <div style={{ marginBottom: '10px' }}>
        <label style={{ color: 'var(--text-color)' }}>Tamanho: </label>
        <input
          type="number"
          value={passwordLength}
          onChange={(e) => setPasswordLength(Number(e.target.value))}
...
const CredenciaisList = () => {
  ...
  if (loading) return <div style={{ color: 'var(--text-color)' }}>Carregando...</div>;
  if (error) return <div style={{ color: 'var(--text-color)' }}>Erro: {error}</div>;
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <div style={{ 
          ...
          {filteredCredentials.length > 0 ? (
            filteredCredentials.map((cred) => (
              <div 
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  backgroundColor: selectedCredential?.name === cred.name ? 'var(--hover-bg)' : 'transparent',
                  transition: 'background-color 0.2s',
                  display: 'flex',
                  justifyContent: 'space-between',
                  alignItems: 'center',
            ...
            <p style={{ color: 'var(--text-color)' }}>{searchTerm ? 'Nenhuma credencial encontrada.' : 'Nenhuma credencial cadastrada.'}</p>
          )}
        </div>
        ...
        <div style={{ 
          ...
          {isAdding || isEditing ? (
            <div>
              <h3 style={{ color: 'var(--text-color)' }}>{isEditing ? 'Editar Credencial' : 'Adicionar Nova Credencial'}</h3>
              {error && <div style={{ color: '#e74c3c', marginBottom: '10px', padding: '10px', backgroundColor: '#fdecea', borderRadius: '4px' }}>{error}</div>}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                {[
                  ...
                  <div key={name} style={{ display: 'flex', alignItems: 'center' }}>
                    <label style={{ width: '100px', textAlign: 'right', marginRight: '10px', color: 'var(--text-color)' }}>
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ color: 'var(--text-color)' }}>{selectedCredential.name}</h3>
                <div>
                  <button 
                    onClick={handleEditCredential} 
                    title="Editar"
      ...
      {showPasswordModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3>Alterar Senha</h3>
            {apiError && <div style={{ color: '#e74c3c', marginBottom: '10px', padding: '10px', backgroundColor: '#fdecea', borderRadius: '4px' }}>{apiError}</div>}
            
            <div style={{ marginBottom: '15px' }}>
              <label style={{ color: 'var(--text-color)' }}>Senha atual:</label>
              <input
                type="password"
                value={editData.currentPassword}
                onChange={(e) => setEditData({...editData, currentPassword: e.target.value})}
            ...
            <div style={{ marginBottom: '15px' }}>
              <label style={{ color: 'var(--text-color)' }}>Nova senha:</label>
              <input
                type="password"
                value={editData.newPassword}
                onChange={(e) => setEditData({...editData, newPassword: e.target.value})}
            ...
            <div style={{ marginBottom: '15px' }}>
              <label style={{ color: 'var(--text-color)' }}>Confirmar nova senha:</label>
              <input
                type="password"
                value={editData.confirmPassword}
                onChange={(e) => setEditData({...editData, confirmPassword: e.target.value})}
      ...
      {showDeleteAccountModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3>Confirmar Exclusão de Conta</h3>
            <p style={{ marginBottom: '15px', color: 'var(--text-color)' }}>Esta ação é irreversível! Todos os seus dados serão permanentemente removidos.</p>
            
            {apiError && <div style={{ color: '#e74c3c', marginBottom: '10px', padding: '10px', backgroundColor: '#fdecea', borderRadius: '4px' }}>{apiError}</div>}
            
            <div style={{ marginBottom: '15px' }}>
              <label style={{ color: 'var(--text-color)' }}>Digite sua senha para confirmar:</label>
              <input
                type="password"
                value={deletePassword}
                onChange={(e) => setDeletePassword(e.target.value)}
